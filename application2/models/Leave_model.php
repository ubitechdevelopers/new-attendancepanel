  ...
  public function leavereport($postData = Null)
  {
    ...
    foreach ($query->result() as $row) {
      ...
      /*  $searchQuery = "";
				 ...
				 $query = $this->db->query("SELECT COUNT(EM.Id) as allcount FROM  EmployeeMaster EM,`Organization` O WHERE O.Id='$orgid' AND EM.OrganizationId='$orgid'")->result();
		         $totalRecords = $query[0]->allcount;
				 
				 if($searchQuery != '')
		         $query = $this->db->query("SELECT COUNT(EM.Id) as allcount FROM EmployeeMaster EM,`Organization` O WHERE O.Id='$orgid' AND EM.OrganizationId='$orgid'  $searchQuery")->result();
		         $totalRecordwithFilter = $query[0]->allcount; 
		        /*  var_dump($totalRecordwithFilter);
				 die(); */
      ...
      foreach ($query11->result() as $row) {
        ...
        $query = $this->db->query("select ApprovalStatus from AppliedLeave where EmployeeId = $row->Id  and ApprovalStatus = 2 And Date BETWEEN '$startdate' And '$enddate'")->result();
        ...
        if ($data['allocated'] == "") {
          ...
          $data['allocated'] = getBalanceLeave($orgid, $row->Id, $date);
        }
