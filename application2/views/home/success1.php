	 ...
	 $ci =& get_instance();
		...
		$query = $ci->db->query("SELECT `Order_id`  FROM `addons_master` WHERE  Order_id ='$order_id'");
			if($ci->db->affected_rows() > 0){
				...
				echo "Addon Already Purchased Please login in again to view Addon.";

				return false;
			}
	

	 $query = $ci->db->query("INSERT INTO `addons_master`(Addon_name,OrganizationId,created_date,AddonEnd_date,Order_id,Payment_id) VALUES (?,?,?,?,?,?)",(array($addonname,$orgid,$createddate,$AddonEnddate,$order_id,$payment_id)));
	 // echo $bulk_attprice;
