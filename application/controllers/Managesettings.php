...
public function showTMPDep($id)
	{
		$data = array('id'=>$id);
		
		$this->load->view('home/showTMPDep',$data);
	}
	public function getEmpotTmpDep()
	{
		$data=$this->manage_model->getEmpotTmpDep();
	}
...
{
		$this->load->model('Manage_model');
		$employeearray = $this->Manage_model->SaveEmpshiftList();
...
{
		$this->load->model('manage_model');
		$employeearray = $this->manage_model->getemployebyshift();
		echo json_encode($employeearray);
}
