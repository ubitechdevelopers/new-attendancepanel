<?php
   ...
   class Manage_model extends CI_Model {
       ...
       public function createdpt() {
           ...
           $dup=$this->db->query("SELECT count(*) as allcount  FROM DepartmentMaster WHERE name='$dna' and OrganizationId=$orgid")->result();
           $totalRecords = $dup[0]->allcount;
   if($totalRecords>1){
       ...
       $q = $this->db->query("INSERT INTO DepartmentMaster(Name,OrganizationId) VALUES ('$dna','$orgid')");
    	...
    	public function getEmpotTmpDep(){
    ...
    public function getAllShift($postData = Null)
    {
        ...
        <li class="upDept hover" style="height:30px;" data-toggle="modal" data-target="#assign"onclick="angular.element(this).scope().GetEmpList(\'' . $row->Id . '\')" 
         ...
         <li class="upDept hover" style="height:30px;" data-toggle="modal" data-target="#assign1"onclick="angular.element(this).scope().GetEmpList(\'' . $row->Id . '\')" 
         data-did="' . $row->Id . '" data-name="' . $row->Name . '"><img src=' . URL . "../assets/img/check-circle.svg" . '  class="#" style="font-size:16px; font-weight: 500; color:#212529; margin-left:18px;" onclick="angular.element(this).scope().GetEmpList(\'' . $row->Id . '\')" data-did="' . $row->Id . '" data-name="' . $row->Name . '"title="Assign"></img> <a href="#" style="color:black; font-size:13px;font-weight: 500; margin-left:8px; ">Assign</a></li>
        </ul>
        </div> ';
            }
    ...
    public function delshiftsss()
    {
        ...
        if ($data['attn'] == 0 and $data['emp'] == 0 and $data['arc'] == 0 and $data['shiftp'] == 0)
        {
            ...
            if ($data['afft'] > 0)
            {
                $query = $this
                    ->db
                    ->query("DELETE FROM `ShiftMasterChild` where ShiftId=? and OrganizationId=?", array($sid,$orgid));
    ...
    public function SaveEmpshiftList()
    {
        ...
        for ($i = 0;$i < count($emplistarr);$i++)
        {
            if ($emplistarr[$i]['sts'] == 1)
            {
               print_r($emplistarr);
                ...
                $query = $this->db->query($sql, array($shifttid,$orgid,$empid));
       ...
       public function createdsg() {
        ...
        $orgid = $_SESSION['orgid'];
       $dup=$this->db->query("SELECT count(*) as allcount  FROM DesignationMaster  WHERE name='$dna' and OrganizationId=$orgid")->result();
        $totalRecords=$dup[0]->allcount;
   ...
   if($totalRecords>1){
    ...
    echo $totalRecords;
       ...
       public function createholiday() {
         ...
         $dup=$this->db->query("SELECT count(*) as allcount FROM HolidayMaster WHERE name='$name'  and OrganizationId=$orgid")->result();
         $totalRecords = $dup[0]->allcount;
   ...
   if($dup->num_rows()>0){
       ...
       $q1.= " AND archive = 0 ";
           ...
           if($searchValue != ''){
             ...
             $query = $this->db->query("SELECT COUNT(D.Id) as allcount FROM DepartmentMaster D where `OrganizationId`='$orgid' $q1")->result();
           $totalRecords = $query[0]->allcount;
   	 
   	     if($searchQuery != '')
           $query = $this->db->query("SELECT COUNT(D.Id) as allcount FROM DepartmentMaster D where`OrganizationId`='$orgid' $searchQuery $q1")->result();
           $totalRecordwithFilter = $query[0]->allcount; 
            //  var_dump($totalRecordwithFilter);
            //  die();
           $query = $this->db->query(" SELECT `Id`, `Name`, `archive` FROM DepartmentMaster D WHERE OrganizationId= $orgid $searchQuery $q1 order by name $columnSortOrder, D.Name ASC Limit $start,$rowperpage");
           //  var_dump($this->db->last_query());
           // die(); 
           $d = array();
           $res = array();
       ...
       public function getAllDesg($postData = Null) {
           ...
           $searchQuery = "";
				 ...
				 $query = $this->db->query("SELECT COUNT(D.Id) as allcount FROM `DesignationMaster` D where `OrganizationId`='$orgid' $q1")->result();
		         $totalRecords = $query[0]->allcount;
				 
				 if($searchQuery != '')
		         $query = $this->db->query("SELECT COUNT(D.Id) as allcount FROM `DesignationMaster` D where`OrganizationId`  ='$orgid' $searchQuery $q1")->result();
		         $totalRecordwithFilter = $query[0]->allcount; 
            
           $query = $this->db->query("SELECT `Id`, `Name`,Description, `CreatedDate`,  `LastModifiedDate`,`archive` FROM `DesignationMaster` D  WHERE OrganizationId= $orgid $searchQuery $q1 order by name $columnSortOrder,D.Name ASC Limit $start,$rowperpage");
       ...
       public function getAllHolidays($postData = null) {
               ...
               if($row->fromDate > date("Y-m-d"))
          {
           ...
           <li class="delete hover" style="height:30px;" data-toggle="modal" data-target="#delete"  
              data-sid="'.$row->Id.'" data-sname="'.$row->Name.'" ><div data-sid="'.$row->Id.'" data-sname="'.$row->Name.'"  title="Delete Client"></div> <img src=' . URL . "../assets/img/trash-2.svg" . '  class="#" style="font-size:16px; font-weight: 500; color:#212529; margin-left:12px;"><a href="#" style="color:black; font-size:13px; margin-left:5px;">Delete</a></li> 
              </ul>
                  </div> '; 
                } 
   else
          {
           ...
           <li class="delete cant23 hover" style="height:30px;" data-toggle="modal" data-target=""  
              data-sid="'.$row->Id.'" data-sname="'.$row->Name.'" ><div data-sid="'.$row->Id.'" data-sname="'.$row->Name.'"  title="Delete Client"></div><img src=' . URL . "../assets/img/trash-2.svg" . '  class="#" style="font-size:16px; font-weight: 500; color:#212529; margin-left:12px;"><a href="#" style="color:black; font-size:13px; margin-left:5px;"> <a href="#" style="color:black; font-size:13px; margin-left:5px;">Delete</a></li> 
   ...
   //
      public function deleteShift(){
       ...
       public function getAttendanceStatusBYAreaId($areaid) {
        $orgid = $_SESSION['orgid'];
           //echo "Select count(Id) as count from AttendanceMaster where areaId= $areaid and OrganizationId = $orgid";exit;
         
          $query = $this->db->query("Select count(Id) as count from AttendanceMaster where areaId= $areaid and OrganizationId = $orgid");
        //   $query = $this->db->query("Select count(Id) as count from EmployeeMaster where area_assigned= $areaid and OrganizationId = $orgid");
           // var_dump($this->db->last_query());
           // die;
           $data = 0;
           if ($row = $query->row()) {
