...
function __construct()
    {
	  ...
	  public function updatePassword(){
		...
		public function getallactivity(){
		 ...
		 $query = $this->db->query("SELECT  Id ,LastModifiedDate AS LastModifiedDate, Module, LastModifiedById,ActionPerformed, OrganizationId , ActivityBY, adminid FROM  ActivityHistoryMaster where OrganizationId = ? AND ActivityBy = 1 AND DATE(LastModifiedDate ) IN ( ".$range." )   ORDER BY LastModifiedDate  DESC Limit 5 ",array($orgid));
		}
		 // var_dump($this->db->last_query());
		else{
			 ...
			 if($rangedate == "")
			   ...
			   $rangedate = 1;
				 ...
				 $query = $this->db->query("SELECT  Id ,LastModifiedDate AS LastModifiedDate, Module, LastModifiedById,ActionPerformed, OrganizationId , ActivityBY, adminid FROM  ActivityHistoryMaster where OrganizationId = ? AND ActivityBy = 1 AND DATE(LastModifiedDate ) IN ( ".$range." )  ORDER BY LastModifiedDate DESC Limit 10 ",array($orgid));
		}
