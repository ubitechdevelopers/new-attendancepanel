...
class Clientmodel extends CI_Model
{
        ...
        foreach ($query->result() as $row) {
            ...
            if ($row->checkin_img != "") {
                ...
                $data['entryimg'] =/*'<img src="'.$row->checkin_img.'"  style="width:60px!important; "/>';*/
                    $data['test'] = '<i class="fa fa-location-arrow fa-2x" style="margin-left:0.5rem;color:#3190E8;" data-toggle="modal"  data-target="#locationmodal" data-latit="' . $row->latit . ',' . $row->longi . '" data-latitin="' . $row->latit . '" data-longiin="' . $row->longi . '" data-checkinloc="' . encode5t($row->location) . '"   title="' . $row->location . '" data-toggle="tooltip" data-placement="top"></i></button>';

                $data['tif'] = substr($row->time, 0, 5) . "<br><small></small>" . '<br><i class="pop" data-toggle="modal" data-target="#imagemodal" data-enimage="' . $row->checkin_img . '"><img src="' . getPresignedURL(ATTENDANCEBUCKET, $row->checkin_img) . '" class="rounded-circle" style="width:42px!important;height:42px!important;border-radius:50%;object-fit:cover; "  /></i>' . '<br>' . $data['test'];

                /*'<i class="pop" data-toggle="modal" data-target="#imagemodal"><img src="'.$imgurl.'"  style="width:60px!important; "/></i>';*/
            } else {
                $data['entryimg'] =
                    $data['test'] = '<i class="fa fa-location-arrow fa-2x" style="margin-left:0.5rem;color:#3190E8;" data-target="#locationmodal"  data-target="#locationmodal" data-latit="' . $row->latit . ',' . $row->longi . '" data-latitin="' . $row->latit . '" data-longiin="' . $row->longi . '" data-checkinloc="' . encode5t($row->location) . '" data-toggle="modal"  title="' . $row->location . '" data-toggle="tooltip" data-placement="top"></i></button>';

                $data['tif'] = substr($row->time, 0, 5) . "<br><small></small>" . '<br><i class="pop" data-toggle="modal" data-target="#imagemodal" data-enimage="http://ubiattendance.zentylpro.com/ubiattnew2/assets/image/avtarprofile.png"><img src="http://ubiattendance.zentylpro.com/ubiattnew2/assets/image/avtarprofile.png" class="rounded-circle" style="width:42px!important;height:42px!important;border-radius:50%;object-fit:cover; "  /></i>' . '<br>' . $data['test'];
            ...
            if ($row->checkout_img != "") {
                ...
                $data['exitimg'] =/*'<img src="'.$row->checkout_img.'"  style="width:60px!important; "/>';*/
                    $data['test1'] = '<i class="fa fa-location-arrow fa-2x" style="margin-left:0.5rem;color:#FF0000;" data-target="#locationmodal" data-target="#locationmodal" data-latit="' . $row->latit . ',' . $row->longi . '" data-latitin="' . $row->latit . '" data-longiin="' . $row->longi . '" data-checkinloc="' . encode5t($row->location) . '" data-toggle="modal"  title="' . $row->location_out . '" data-toggle="tooltip" data-placement="top"></i></button>';

                $data['tof'] = substr($row->time_out, 0, 5) . "<br><small></small>" . '<br><i class="pop" data-toggle="modal" data-target="#imagemodal" data-enimage="' . $row->checkout_img . '"><img src="' . getPresignedURL(ATTENDANCEBUCKET, $row->checkout_img) . '" class="rounded-circle" style="width:42px!important;height:42px!important;border-radius:50%;object-fit:cover; "  /></i>' . '<br>' . $data['test1'];
                /*'<i class="pop" data-toggle="modal" data-target="#imagemodal"><img src="'.$imgurl.'"  style="width:60px!important; "/></i>';*/
            } else {

                $data['exitimg'] =
                    /*'<img src="'.$row->checkout_img.'"  style="width:60px!important; "/>';*/
                    $data['test1'] = '<i class="fa fa-location-arrow fa-2x" style="margin-left:0.5rem;color:#FF0000;" data-target="#locationmodal" data-target="#locationmodal" data-latit="' . $row->latit . ',' . $row->longi . '" data-latitin="' . $row->latit . '" data-longiin="' . $row->longi . '" data-checkinloc="' . encode5t($row->location) . '"  data-toggle="modal"  title="' . $row->location_out . '" data-toggle="tooltip" data-placement="top"></i></button>';

                $data['tof'] = substr($row->time_out, 0, 5) . "<br><small></small>" . '<br><i class="pop" data-toggle="modal" data-target="#imagemodal" data-enimage="http://ubiattendance.zentylpro.com/ubiattnew2/assets/image/avtarprofile.png"><img src="http://ubiattendance.zentylpro.com/ubiattnew2/assets/image/avtarprofile.png" class="rounded-circle" style="width:42px!important;height:42px!important;border-radius:50%;object-fit:cover; "  /></i>' . '<br>' . $data['test1'];
            ...
            //     $data['action']='';
            // }
            $data['action'] = '<a href="trackempvisit/' . $data['eid'] . '/' . $data['date2'] . '/' . $data['timein'] . '/' . $data['timeout'] . '/' . $data['total'] . '" target="_self"><i class="fa fa-street-view" style="font-size:24px; color:black;" title="Track punched visits"></i></a>';
    ...
    function getClientData($postData = Null)
    {
        ...
        if ($statusfilter == 1 || $statusfilter == '') {
            ...
            $query = $this->db->query("SELECT COUNT(CM.Id) as allcount FROM ClientMaster CM  where `OrganizationId`='$orgid' $q1 AND `status` = 1")->result();
            $totalRecords = $query[0]->allcount;

            if ($searchQuery != '')
                $query = $this->db->query("SELECT COUNT(CM.Id) as allcount FROM ClientMaster CM where`OrganizationId`='$orgid' $q1 AND `status` = 1 $searchQuery")->result();
            $totalRecordwithFilter = $query[0]->allcount;
            //   var_dump($totalRecordwithFilter);
            //  die();
            $query = $this->db->query("SELECT id,(select count(C.id)as ids from clientlist C where C.clientid = CM.Id) as coutnemp,Company,Name,Contact,Email,Address,City,Country,Description,status,Zone FROM ClientMaster CM WHERE OrganizationId=$orgid  $q1 AND `status` = 1 $searchQuery order by Name $columnSortOrder,CM.Company ASC  Limit $start,$rowperpage ");
        } else {
            ...
            $query = $this->db->query("SELECT COUNT(CM.Id) as allcount FROM ClientMaster CM  where `OrganizationId`='$orgid' AND `status` = 0")->result();
            $totalRecords = $query[0]->allcount;

            if ($searchQuery != '')
                $query = $this->db->query("SELECT COUNT(CM.Id) as allcount FROM ClientMaster CM where`OrganizationId`='$orgid' AND `status` = 0 $searchQuery")->result();
            $totalRecordwithFilter = $query[0]->allcount;
            //   var_dump($totalRecordwithFilter);
            //  die();
            $query = $this->db->query("SELECT id,(select count(C.id)as ids from clientlist C where C.clientid = CM.Id) as coutnemp,Company,Name,Contact,Email,Address,City,Country,Description,status,Zone FROM ClientMaster CM WHERE OrganizationId=$orgid  AND `status` = 0 $searchQuery order by Name $columnSortOrder,CM.Company ASC  Limit $start,$rowperpage ");
        }
        ...
        $res = array();
        $d = array();
        // $query = $this->db->query("SELECT id,(select count(C.id)as ids from clientlist C where C.clientid = CM.Id) as coutnemp,Company,Name,Contact,Email,Address,City,Country,Description,status,Zone FROM ClientMaster CM WHERE OrganizationId=$orgid  AND `status` = 1 $searchQuery order by Name $columnSortOrder,CM.Name ASC  Limit $start,$rowperpage " );
        foreach ($query->result() as $row) {
            $data = array();
            ...
            if ($data['sts'] == '0') {
                ...
                $data['thump'] = '<i class="  statusactive fa fa-thumbs-up" style="font-size:16px; color:black" data-toggle="modal" data-id="' . $row->id . '" data-cname="' . $row->Company . '" data-sts="' . $row->status . '" data-ena="' . $row->Name . '" title="Make Active"></i>';
            } else {
                $data['thump'] = '<i  class=" statusinactive fa fa-thumbs-down" style="font-size:16px; color:black" data-toggle="modal" data-id="' . $row->id . '" data-cname="' . $row->Company . '" data-sts="' . $row->status . '" data-countsts="' . $row->coutnemp . '" data-ena="' . $row->Name . '" title="Make Inactive"></i>';
            }
            ...
            $data['action'] = '<div class="dropdown">
                                     ...
                                     <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel" style="min-width: 6.3rem; height:6.5rem; border-radius:5px; padding: 0.7rem 0.5rem 0.5rem 0.5rem;margin-left:-25%">
                                        ...
                                        <li class="edit" style="height:30px;" data-toggle="modal" data-target="#addClientE"  
                 data-id="' . $row->id . '"
                data-comp="' . $row->Company . '"
                data-name="' . $row->Name . '"
                data-email="' . $row->Email . '"
                data-addr="' . $row->Address . '"
                 data-city="' . $row->City . '"
                 ...
                 data-desc="' . $row->Description . '"
                 data-zone="' . $row->Zone . '"
                data-sts="' . $row->status . '"><i id="get" class="fa fa-pencil  material-icons  mod" style="font-size:16px; margin:0px 8px 0px 6px;"  title="Edit"></i><a href="#" style="color:black; font-size:13px; margin-left:5px;">Edit</a></li> 
   
   
                <li class="delete" style="height:30px;" data-toggle="modal" data-target="#delClient"  
                 data-id="' . $row->id . '" data-cname="' . $row->Company . '" data-ena="' . $row->Name . '" data-countsts="' . $row->coutnemp . '"><i class=" delete fa fa-trash" style="font-size:16px; margin:0px 8px 0px 6px;"  data-id="' . $row->id . '" data-cname="' . $row->Company . '" data-ena="' . $row->Name . '" data-countsts="' . $row->coutnemp . '"  title="Delete Client"></i> <a href="#" style="color:black; font-size:13px; margin-left:5px;">Delete</a></li>
   
                 <li class="Status" style="height:30px;" data-toggle="modal" data-target="#changestatus">' . $data['thump'] . '<a href="#" style="color:black; font-size:13px; margin-left:5px;">    &nbsp&nbspStatus</a></li></div>';
    ...
    public function emportCli($request)
    {
        ...
        //var_dump($location);
        //die;
       // $location = "http://ubiattendance.zentylpro.com/ubiattnew2/uploads/employee/10/";
        //var_dump($location);
        $fp = $location . $file_name;
        $sts = 1;
        $i = 0;
...
//    var_dump($data[$phone]);
   ...
   (?,?,?,?,?,?,?,?,?,?,?,?)";
                            $this->db->query($sql, array(
                                $data[$clientname], $data[$company],
                                $data[$email], $data[$desc], $data[$city], $data[$address], $data[$phone], $sts, $orgid, $adminid, $date, $data[$zonecitye]
                            ));

                            
                            // echo "INSERT INTO ClientMaster (Name,Company,Email,Description,City,Address,Contact,Status,
                            // OrganizationId,createdBy,ModifiedDate,Zone) VALUES
                            // ('$clientname','$company','$email','$desc','$city','$address','$phone','$sts','$orgid','$adminid','$date','$zonecitye')";
    ...
    function getClientlist($postData = Null)
    {
        $orgid = $_SESSION['orgid'];
        ...
        if ($searchValue != '') {
            $searchQuery = "  And (CM.Company like '%" . $searchValue . "%') ";
        }
        ...
        if ($searchQuery != '')
            $query = $this->db->query("SELECT COUNT(CM.Id) as allcount FROM ClientMaster CM,clientlist S where S.organizationid ='$orgid'  and  S.clientid=CM.Id $searchQuery  group by S.employeeid ");
        $totalRecordwithFilter = $query->num_rows();

        //    var_dump($totalRecordwithFilter);
        //    die() ;    

        $query = $this->db->query("SELECT S.clientid,S.employeeid,GROUP_CONCAT(CM.Company SEPARATOR ', ') as compname ,GROUP_CONCAT(S.clientid SEPARATOR ', ') as `cid` from clientlist S, ClientMaster CM where S.clientid= CM.Id AND S.organizationid='$orgid' $searchQuery   Group by S.employeeid Limit $start,$rowperpage ");
        ...
        foreach ($query->result() as $row) {
            ...
            $data['action'] = '<div class="dropdown">
                                     ...
                                     <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel" style="min-width: 5.3rem; height:5rem; border-radius:5px; padding: 0.7rem 0.5rem 0.5rem 0.5rem;margin-left:-25%">
   
                                        <li style="height:30px;" data-toggle="modal" 
             <a href="#" ><i id="showclientlist" class="fa fa-eye" "style="font-size:13px; color="black" data-toggle="modal" title="View"
              data-eid="' . $row->employeeid . '"
              data-client="' . $row->clientid . '"
              data-name="' . $data['empname'] . '"
              data-target="#editclient">&nbsp&nbspView</i></a></li>
                <li style="height:10px;" data-toggle="modal"  
            <a href="#"><i id="getempnameforclient" class="getassignclient fa fa-tasks" style="font-size:14px; color:black;" data-toggle="modal" title="Assign"
